...
class PoemController {
    ...
    async getPoemBySlug(req, res) {
        try {
            const { slug } = req.params;
            const result = await this.db.query('SELECT p.*, c.name as category_name FROM poems p LEFT JOIN categories c ON p.category_id = c.id WHERE p.slug = $1 AND p.is_published = true', [slug]);
    ...
    async createPoem(req, res) {
        try {
            const { title_bengali, title_english, content, description, category_id, is_published, publish_date, } = req.body;
            const slug = title_english?.toLowerCase().replace(/\s+/g, '-') || title_bengali.toLowerCase();
            const result = await this.db.query(`INSERT INTO poems (
    ...
    async updatePoem(req, res) {
        try {
            ...
            const result = await this.db.query(`UPDATE poems SET
        ...
        WHERE id = $7 RETURNING *`, [title_bengali, title_english, content, description, is_published, publish_date, id]);
            if (result.rows.length === 0) {
                return res.status(404).json({ error: 'Poem not found' });
            }
            res.json({ data: result.rows[0] });
...
exports.PoemController = PoemController;
//# sourceMappingURL=poemController.js.map